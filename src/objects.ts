...
export function isValid(question: Question, answer: string): boolean {
    ...
    return question.options.reduce(
        (previousValue: boolean, option: string): boolean =>
            previousValue || option == answer,
        false,
    );
}
...
export function toMarkdown(question: Question): string {
    let markdownString: string = `# ${question.name}\n`;
...
/**
 * Returns a deep copy of a Question object
 * @param question the Question object to copy
 */

function deepCopy(question: Question): Question {
    return {
        id: question.id,
        name: question.name,
        type: question.type,
...
export function renameQuestion(question: Question, newName: string): Question {
    let copy: Question = deepCopy(question);
    copy.name = newName;
    return copy;
}
...
export function publishQuestion(question: Question): Question {
    let copy: Question = deepCopy(question);
    copy.published = !question.published;
    return copy;
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    let copy: Question = deepCopy(oldQuestion);
    copy.id = id;
...
export function mergeQuestion(
    ...
    let copy: Question = deepCopy(contentQuestion);
    copy.id = id;
    copy.name = name;
    copy.points = points;
    copy.published = false;
